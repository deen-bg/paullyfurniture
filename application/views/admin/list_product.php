...
<head>
	...
	<title>Product | Paully Furniture </title>
			                  				...
			                  				<table id="exampleTableToolbar" data-mobile-responsive="true" class="table table-hover" data-pagination="true" data-search="true" style="margin-top: 0px;">
			                    				<thead style="">
			                    					<tr>
														...
														<th style="" data-field="Category" data-align="center">
			                    						...
			                    						<th style="" data-field="Created" data-align="center">
			                    							<div class="th-inner ">Update at</div>
			                    							<div class="fht-cell"></div>
			                    						</th>
														<th style="" data-field="Recommend" data-align="center"  data-toggle="tooltip" data-placement="top" title="สินค้าแนะนำ">
			                    							<div class="th-inner ">Recommend</div>
			                    							<div class="fht-cell"></div>
			                    						</th>
														<th style="" data-field="New" data-align="center"  data-toggle="tooltip" data-placement="top" title="สินค้าใหม่">
			                    				...
			                    				<?php if(!empty($products)) : ?>
			                  					<tbody>
													...
													?>
			                  						<tr> 
			                  							<td class=""><?=$i+1; ?></td> 
			                  							<td style=""><?=$val_item['p_name']; ?></td>
														<td style=""><?=$val_item['c_name']; ?></td>
														<td style=""><?=number_format($val_item['price'], 2); ?></td>
														<td style="">
															<a type="button" href="<?=base_url('Admin/albums/'.$val_item['p_id'].'/?slug='.$val_item['p_name']);?>" class="btn btn-outline-primary" data-toggle="tooltip" data-placement="top" title="File upload">
															<i class="fa fa-picture-o" aria-hidden="true"></i>
														  	</a>
														</td>
			                  							<td style=""><?=date("d/m/Y", strtotime($val_item['update_date'])); ?></td>
														<td style="">
															<?php if ($val_item['is_recommend'] == '1') : ?>
			                  									<button class="btn btn-sm btn-success btn-icon btn-floating" type="button" onclick="sChange(<?=$val_item['p_id']; ?>, 0)"  data-toggle="tooltip" data-placement="top" title="YES"><i class="icon wb-check" aria-hidden="true"></i></button>
			                  								<?php else : ?>
			                  									<button class="btn btn-sm btn-danger btn-icon btn-floating" type="button" onclick="sChange(<?=$val_item['p_id']; ?>, 1)" data-toggle="tooltip" data-placement="top" title="NO"><i class="icon wb-close" aria-hidden="true"></i></button>
			                  								<?php endif; ?>
			                  							</td>
														<td style="">
															<?php if ($val_item['is_new'] == '1') : ?>
																<button class="btn btn-sm btn-success btn-icon btn-floating" type="button" onclick="newChange(<?=$val_item['p_id']; ?>, 0)"  data-toggle="tooltip" data-placement="top" title="YES"><i class="icon wb-check" aria-hidden="true"></i></button>
			                  								<?php else : ?>
																<button class="btn btn-sm btn-danger btn-icon btn-floating" type="button" onclick="newChange(<?=$val_item['p_id']; ?>, 1)" data-toggle="tooltip" data-placement="top" title="NO"><i class="icon wb-close" aria-hidden="true"></i></button>
			                  								<?php endif; ?>
			                  							</td>
														<td style="">
			                  							...
			                  							<td>
			                  								<button type="button" class="btn btn-round btn-warning btn-sm" onclick="window.location.href = '<?=base_url('Admin/edit_product/'.$val_item['p_id']);?>';" data-toggle="tooltip" data-placement="top" title="Edit"><i class="icon wb-pencil" aria-hidden="true"></i></button>
			                  								<button type="button" class="btn btn-round btn-danger btn-sm" onclick="delServ(<?=$val_item['p_id'];?>, <?=$val_item['is_active'];?>)" data-toggle="tooltip" data-placement="top" title="Setting"><i class="fa fa-cog" aria-hidden="true"></i></button>
			                  							</td> 
			                  						</tr>
			                  						<?php
			                  							$i++;
	...
	<div class="modal fade bs-example-modal-sm" id="modalIsNew" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog modal-md">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
					<h4 class="modal-title" id="myModalLabel2">สินค้าใหม่</h4>
				</div>
				<div class="modal-body">
					<h4>คุณต้องการเปลี่ยนสถานะ ใช่หรือไม่ ?</h4>
					<h6>สามารถ เปิด/ปิด รายการสินค้าใหม่ได้ทุกเมื่อ</h6>
				</div>
				<div class="modal-footer">
					<input type="hidden" id="p_id" name="p_id">
                    <input type="hidden" id="p_st" name="p_st">
